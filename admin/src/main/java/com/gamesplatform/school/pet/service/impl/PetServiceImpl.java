package com.gamesplatform.school.pet.service.impl;

import com.gamesplatform.school.pet.service.PetService;

import com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper;
import com.gamesplatform.common.BusinessException;
import com.gamesplatform.school.pet.dto.*;
import com.gamesplatform.school.pet.entity.*;
import com.gamesplatform.school.pet.mapper.*;
...
public class PetServiceImpl implements PetService {
     ...
     * @param useAll 是否按需批量使用。
     * @return 处理结果。
     */
    @Transactional
    @Override
    public PetUseBenefitResponse useBenefit(Long userId, Long userBenefitId, boolean useAll) {
        ...
        if (useAll) {
            if (!TYPE_CONSUMABLE.equals(userBenefit.getBenefitType())) {
                throw new BusinessException("永久权益不支持一键使用");
            }
            usedQuantity = calculateUseQuantity(pet, config, remainingQuantity);
        }
        applyBenefitEffect(pet, config, usedQuantity);
    ...
    private void applyBenefitEffect(PetUser pet, PetBenefitConfig config, int quantity) {
        int effectValue = (config.getEffectValue() != null
                ? config.getEffectValue()
                : BENEFIT_STATUS_ADD_VALUE) * quantity;
        switch (config.getEffectType()) {
            case "HUNGER_FULL" -> {
                validateStatusCanIncrease(pet.getHunger(), "饥饿值");
                pet.setHunger(addStatusValue(pet.getHunger(), effectValue));
            }
            case "CLEAN_FULL" -> {
                validateStatusCanIncrease(pet.getClean(), "清洁值");
                pet.setClean(addStatusValue(pet.getClean(), effectValue));
            }
            case "HAPPINESS_FULL" -> {
                validateStatusCanIncrease(pet.getHappiness(), "快乐值");
                pet.setHappiness(addStatusValue(pet.getHappiness(), effectValue));
            }
            case "ENERGY_FULL" -> {
                validateStatusCanIncrease(pet.getEnergy(), "体力值");
                pet.setEnergy(addStatusValue(pet.getEnergy(), effectValue));
            }
            case "EXP_ADD" -> addExp(pet, effectValue);
    ...
    private int calculateUseQuantity(PetUser pet, PetBenefitConfig config, int availableQuantity) {
        int currentValue = switch (config.getEffectType()) {
            case "HUNGER_FULL" -> pet.getHunger() != null ? pet.getHunger() : 0;
            case "CLEAN_FULL" -> pet.getClean() != null ? pet.getClean() : 0;
            case "HAPPINESS_FULL" -> pet.getHappiness() != null ? pet.getHappiness() : 0;
            case "ENERGY_FULL" -> pet.getEnergy() != null ? pet.getEnergy() : 0;
            default -> throw new BusinessException("该道具不支持一键使用");
        };
        validateStatusCanIncrease(currentValue, statusName(config.getEffectType()));
        int effectValue = config.getEffectValue() != null
                ? config.getEffectValue()
                : BENEFIT_STATUS_ADD_VALUE;
        if (effectValue <= 0) {
            throw new BusinessException("道具效果值配置无效");
        }
        int requiredQuantity = (100 - currentValue + effectValue - 1) / effectValue;
        return Math.min(availableQuantity, requiredQuantity);
    }

    private String statusName(String effectType) {
        return switch (effectType) {
            case "HUNGER_FULL" -> "饥饿值";
            case "CLEAN_FULL" -> "清洁值";
            case "HAPPINESS_FULL" -> "快乐值";
            case "ENERGY_FULL" -> "体力值";
            default -> "状态值";
        };
    }

    private void validateStatusCanIncrease(Integer currentValue, String statusName) {
        if (currentValue != null && currentValue >= 100) {
            throw new BusinessException(statusName + "已达到 100%，不需要使用该道具");
        }
    }
